...
// billing, compliance, or Twilio details. Until staff auth exists, access is
// gated by the same httpOnly account-context cookie used by `/account`, so this
// is effectively an owner-accessible preview of the future staff workspace.
export default async function WorkspacePage() {
  const token = await readAccountSessionToken();
  if (!token) return <WorkspaceGate />;

  const lookup = await lookupSetupRequestByRawToken(token);
  if (!lookup.ok) return <WorkspaceGate />;
...
function WorkspaceGate({ needsSetup = false }: { needsSetup?: boolean }) {
  return (
    <main className="ws-page">
      <section className="card card-pad" style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        <p style={{ marginTop: "var(--space-5)" }}>
          <a className="link" href={needsSetup ? "/account" : "https://missedcallsdental.com/"}>
            {needsSetup ? "Go to your account →" : "Request a new setup link →"}
          </a>
        </p>
      </section>
    </main>
