...
const STATUS: Record<Exclude<Awaited<ReturnType<typeof restoreScheduledPhoneNumber>>, { ok: true }>["error"], number> = {
  not_found: 404,
  already_removed: 409,
  not_scheduled: 409,
...
export async function POST(
  ...
  if (access.membership.role === "front_desk") {
    return jsonForbidden("Front desk users cannot restore account phone numbers.");
  }

  const { phoneNumberId } = await params;
  const result = await restoreScheduledPhoneNumber({
