...
| PA-13 | SMS Conversation Builder v1 | platform-admin/sms-conversation-builder.md | platform_admin | internal | OPERATIONS-RUNBOOK.md (2026-06-12), config/sms-recovery.config.ts, lib/sms-recovery/conversation-templates.ts, lib/sms-recovery/reply-classification.ts, lib/sms-recovery/voice-greeting-templates.ts | /admin | Admin-only deterministic SMS + voice greeting flow; read-only by default with Edit -> Save -> read-only; Voice greeting block first, SMS messages second; full-template Initial SMS editor with server-required clinic identity + STOP opt-out; code defaults are source of truth; `body_text=NULL` means current default; reset/default-equal saves remove overrides instead of storing literal defaults; cleanup migration `20260621000100_clean_sms_template_default_overrides.sql`; up to 3 follow-ups; max 0–3; enabled NULL-body follow-ups send current default; new recorded recovery SMS resets the auto-reply cycle while retaining patient name; deterministic reply classification; conservative patient-name extraction with name-question skip; voice scenarios will_send/duplicate/none; no AI; owners cannot edit; default-safe (0 auto-replies + default voice greetings) |
