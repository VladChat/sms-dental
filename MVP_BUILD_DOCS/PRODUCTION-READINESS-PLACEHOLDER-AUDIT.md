...
| A2P approval review workflow | Implemented; LIVE armed for Fairstone; Fairstone submit blocked by website data | Platform-admin-only minimal review package + one-click real submit (Trust Hub Customer Profile + A2P Trust Product + Brand + Campaign + sender attachment), idempotent/resumable. As of 2026-06-08 `submissionMode="live"`, Fairstone is the only allowlisted clinic, and the primary Customer Profile SID is configured — so the workflow is armed. The provider payload is minimum-information only (optional fields omitted; internal diagnostics not submitted; full EIN never shown/logged/stored). Fairstone's real Submit is currently BLOCKED because its stored website is `allyexp.com` (owner/ISV domain, not the clinic's) — the package blocks rather than submit unrelated data; set the clinic's real website to enable Submit. Clinic owners cannot submit. |
